    ...
    <div>
      ...
      <div className="flex flex-col lg:flex-row lg:gap-0 lg:bg-white lg:border lg:border-mist-200 lg:rounded-2xl lg:overflow-hidden" style={{ height: "calc(100vh - 120px)" }}>
        ...
        <div className={`lg:w-80 lg:flex-shrink-0 lg:border-r lg:border-mist-200 flex flex-col min-h-0 ${selected ? "hidden lg:flex" : "flex"}`}>
          {/* List header */}
          <div className="p-4 border-b border-mist-200 flex-shrink-0">
            <h1 className="text-xl font-bold text-mist-900 mb-3">Conversations</h1>
            <div className="relative">
              <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-mist-400" />
              <input type="text" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)}
                className="w-full pl-8 pr-3 py-2 bg-mist-50 border border-mist-200 rounded-lg text-sm text-mist-900 placeholder:text-mist-400 outline-none focus:border-black transition" />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {loading ? (
              ...
              <div className="p-6 text-center">
                <MessageCircle size={32} className="text-mist-300 mx-auto mb-2" />
                <p className="text-mist-400 text-xs">{sessions.length === 0 ? "No conversations yet" : "No results"}</p>
              </div>
            ) : (
              <div className="space-y-1">
                {filteredSessions.map((s) => (
                  <button
                    ...
                    className={`w-full rounded-xl p-3 text-left hover:bg-mist-50 transition-colors flex items-center gap-3 min-w-0 ${selected?.id === s.id ? "bg-mist-100 border border-mist-200" : "border border-transparent"}`}
                  >
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${s.isPaused ? "bg-yellow-100 text-yellow-700" : "bg-mist-100 text-mist-500"}`}>
                      <MessageCircle size={16} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-0.5">
                        <p className="text-sm font-medium text-mist-900 truncate">{s.visitorName || s.visitorId.slice(0, 20)}</p>
                        <span className="text-[10px] text-mist-400 flex-shrink-0 ml-1">{formatRelative(s.updatedAt)}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        {s.isPaused && <span className="text-[10px] bg-yellow-100 text-yellow-700 px-1.5 py-0.5 rounded font-medium">Live</span>}
                        <p className="text-xs text-mist-400 truncate">{s.lastMessage || "No messages"}</p>
                      </div>
                    </div>
                    <span className="text-[10px] text-mist-400 bg-mist-100 px-1.5 py-0.5 rounded flex-shrink-0">{s._count.messages}</span>
                  </button>
                ))}
              </div>
            )}
        ...
        {/* Detail pane */}
        <div className={`flex-1 flex flex-col min-h-0 min-w-0 ${selected ? "flex" : "hidden lg:flex"}`}>
          {!selected ? (
            <div className="flex-1 flex flex-col items-center justify-center text-mist-300">
            ...
            <>
              {/* Detail header */}
              <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-mist-200 flex-shrink-0 flex-wrap">
                <div className="flex items-center gap-3">
                  <button onClick={() => setSelected(null)} className="lg:hidden text-mist-400 hover:text-mist-700">
                    <ArrowLeft size={20} />
                  </button>
                  <div>
                    <p className="font-semibold text-mist-900 text-sm truncate max-w-[200px]">{selected.visitorName || selected.visitorId.slice(0, 20)}</p>
              ...
              {/* Reply input */}
              {selected.isPaused && (
                <div className="border-t border-mist-200 px-4 py-3 flex-shrink-0">
