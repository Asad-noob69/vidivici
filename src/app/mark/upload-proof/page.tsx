...
export default function MarkUploadProofPage({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/mark/upload-proof?token=${token}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/mark/upload-proof", { method: "POST", body: form })
      ...
      <div className="min-h-screen bg-mist-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow p-8 max-w-md w-full text-center">
          ...
          <h1 className="text-xl font-bold text-mist-900 mb-2">
            {booking?.alreadyUploaded && !success ? "Already Submitted" : "Proof Submitted"}
          </h1>
          <p className="text-mist-500">
            {booking?.alreadyUploaded && !success
              ? "We have already received your wire transfer proof for this booking."
              : "Thank you! We have received your wire transfer proof. Our team will review it and finalize your booking shortly."}
          </p>
          {booking && (
    ...
    <div className="min-h-screen bg-mist-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow p-8 max-w-md w-full">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-mist-900">Upload Wire Transfer Proof</h1>
        ...
        {booking && (
          <div className="bg-mist-50 rounded-xl p-4 mb-6 text-sm space-y-2">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold text-mist-900">${booking.balanceDue?.toLocaleString()}</span>
            </div>
          </div>
        )}

        <p className="text-mist-600 text-sm mb-4">
          Please upload your wire transfer proof. Accepted: bank receipt, SWIFT confirmation, or screenshot. Formats: PDF, JPG, PNG (max 10MB).
        </p>
        ...
        >
          {uploading ? "Uploading..." : "Submit Wire Proof"}
        </button>
      </div>
    </div>
  )
