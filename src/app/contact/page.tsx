...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5">
      {label && <label className="text-[12px] font-semibold text-gray-700">{label}</label>}
      {children}
    </div>
  );
}
...
export default function ContactPage() {
  ...
  const inputCls = "w-full border border-gray-200 rounded-xl px-4 py-2.5 text-[13px] text-gray-900 placeholder-gray-300 outline-none focus:border-gray-400 transition-colors bg-white";
      ...
      <section className="w-full bg-[#F0F1F2] pt-14 pb-96 sm:px-14 md:px-24">
        <div className="mx-auto py-16">
          <div className="text-center mb-10 space-y-8">
            ...
            <p className="mt-2 text-base text-gray-400 leading-relaxed max-w-xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {contactDetails.map((item) => (
              ...
              >
                <div className="w-10 h-10 rounded-full border border-gray-100 bg-gray-50 flex items-center justify-center text-gray-500">
                  {item.icon}
                </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="Phone Number">
                  <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden focus-within:border-gray-400 transition-colors bg-white">
                    <span className="px-3 py-2.5 text-sm border-r border-gray-200 bg-gray-50 text-gray-600 flex-shrink-0">🇺🇸 +1</span>
                    <input name="phone" value={form.phone} onChange={handleChange} placeholder="(555) 555-5555" className="flex-1 px-3 py-2.5 text-[13px] text-gray-900 placeholder-gray-300 outline-none" />
                  </div>
                </Field>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Your Budget">
                  <select name="budget" value={form.budget} onChange={handleChange} className={`${inputCls} ${!form.budget ? "text-gray-400" : "text-gray-900"}`}>
                ...
                <Field label="What do you need for your trip?">
                  <div className="flex items-center gap-4 h-[42px] px-1">
                    {(["House", "Car", "VIP events"] as const).map((need) => (
                      <label key={need} className="flex items-center gap-1.5 cursor-pointer text-[13px] text-gray-700">
