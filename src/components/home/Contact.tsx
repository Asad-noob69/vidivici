...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const category =
        form.inquiryType === "Car Rental" ? "Car"
        : form.inquiryType === "Villa Booking" ? "Villa"
        : form.inquiryType === "VIP Event" ? "Event"
        : "General";
  ...
  return (
    <section className="w-full bg-white sm:px-16 lg:px-20 px-6 2xl:px-32 mt-24 2xl:mt-48">
      <div className="">
        <div className="border border-mist-200 rounded-3xl 2xl:rounded-[48px] overflow-visible gap-8 2xl:gap-14 sm:p-8 2xl:p-14 px-4 py-6 flex flex-col md:flex-row shadow-sm">
          ...
          <div className="bg-mist-100 px-4 sm:px-8 py-8 2xl:py-14 md:w-1/3 2xl:w-[480px] flex-shrink-0 flex flex-col gap-8 2xl:gap-12 relative overflow-hidden rounded-2xl 2xl:rounded-[32px]">
            ...
            <div className="relative z-10 flex flex-col gap-6 2xl:gap-9">
              <ContactInfo icon={<Phone className="w-4 h-4 2xl:w-[1.35rem] 2xl:h-[1.35rem]" />} label="Phone" value="(310) 555-0991" />
              <ContactInfo icon={<Mail className="w-4 h-4 2xl:w-[1.35rem] 2xl:h-[1.35rem]" />} label="Email" value="admin@vidivicitrental.com" />
              <ContactInfo icon={<MapPin className="w-4 h-4 2xl:w-[1.35rem] 2xl:h-[1.35rem]" />} label="Address" value="8687 Melrose Ave, Los Angeles CA 90069, United States" />
              <ContactInfo icon={<Clock className="w-4 h-4 2xl:w-[1.35rem] 2xl:h-[1.35rem]" />} label="Working Hours" value="Mon–Sun: 8 AM – 8 PM" />
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6 2xl:gap-8">
                ...
                <Field label="Inquiry Type">
                  <div className="relative">
                    <select
                      ...
                      onChange={handleChange}
                      className="w-full border border-mist-300 rounded-xl 2xl:rounded-2xl px-4 py-3 pr-12 2xl:px-6 2xl:pr-12 2xl:py-5 text-sm 2xl:text-xl text-mist-900 focus:outline-none focus:border-mist-400 transition-colors duration-200 bg-white appearance-none cursor-pointer"
                      required
                    >
              ...
              <button
                ...
                className="w-full bg-mist-900 text-white font-normal py-4 2xl:py-6 rounded-xl 2xl:rounded-2xl text-sm 2xl:text-xl hover:bg-mist-800 transition-all duration-200 mt-2 active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {isSubmitted ? "Message Sent!" : submitting ? "Sending…" : "Send"}
              </button>
            </form>
          </div>
        </div>
